...
function loadFavorites() {
    const stored = localStorage.getItem('sltv-favorites');
    favorites = stored ? JSON.parse(stored) : {};
}

function saveFavorites() {
    localStorage.setItem('sltv-favorites', JSON.stringify(favorites));
}
...
function getPlaylistUrl() {
    return 'https://iptv-org.github.io/iptv/countries/lk.m3u';
}
...
async function fetchPlaylist() {
    ...
    try {
        ...
        if (channels.length === 0) {
            throw new Error("M3U parsing resulted in an empty list.");
        }

    } catch (error) {
        console.error("Error loading playlist:", error);
        errorMessage.textContent = `Failed to load channels. ${error.message}`;
    ...
    lines.forEach(line => {
        ...
        if (line.startsWith('#EXTINF:')) {
            ...
            if (match) {
                ...
                // Parse attributes like tvg-logo, group-title
                ...
                for (const attr of attrMatches) {
                    ...
                    if (key === 'tvg-logo') {
                        ...
                        currentChannel['group-title'] = value;
                    }
                }
            }
...
function createChannelItem(channel) {
    ...
    const currentProgramHtml = '<span class="italic text-gray-500">Guide information not available</span>';
...
function renderFavorites() {
    ...
    const favChannels = channels.filter(c => favUrls.includes(c.url));
